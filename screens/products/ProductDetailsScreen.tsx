...
import { ProductDetailsProps } from '../../constants/types/products/ProductTypes';
...
export default function ProductDetailsScreen(
  { navigation, route }: ProductDetailsProps,
): React.ReactElement {
  ...
  const colors: Array<string> = route.params?.colors;
  ...
  return (
    <CarouselInfoContext.Provider value={{ state, dispatch } as any}>
        ...
        <View style={{ flex: 1, alignItems: 'center' }}>
          <Carousel
            ...
            data={route.params?.product?.productimage_set.filter(
              ...
              (imageSet: { color: { slug: string } }) => imageSet.color.slug === colors[state.colorSelectedIndex])}
          ...
          <View style={{ flex: 0.8, flexDirection: 'row', alignItems: 'center', marginHorizontal: 15 }}>
            ...
            <View style={{ flex: 1, marginVertical: 10, marginHorizontal: 10 }}>
              <RectButton
                ...
                onPress={() => {
                  navigation.navigate('SizeModal' as any, {
                    parentState: state,
