...
import {
  View, StyleSheet, Text, YellowBox,
} from 'react-native';
import { RectButton, TouchableWithoutFeedback } from 'react-native-gesture-handler';
import { TransitionPresets } from '@react-navigation/stack';

import { SizeModalProps } from '../../constants/types/Types';
...
export default function SizeModalScreen({ navigation, route }: SizeModalProps) {
      ...
      <View style={styles.innerBottomContainer}>
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
          ...
          <View style={{ flexDirection: 'row' }}>
            {sizes.flatMap((size: string, index: number) => (
              <ButtonGroup
                key={index}
                size={size}
                firstChild={index === 0}
